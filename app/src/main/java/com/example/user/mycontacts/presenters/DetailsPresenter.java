...
public class DetailsPresenter implements DetailsContract.Presenter {
    ...
    public void onSendMessageClicked(ContactData contactData) {
        if (Common.checkPermissions(Common.SMS_SEND_PERMISSION,(Activity)mContext )) {
            mContext.startActivity(new Intent(
                    Intent.ACTION_VIEW, Uri.fromParts("sms", contactData.getmPhoneNumber(), null)));

        } else
            Common.verifyPermissions( Common.SMS_SEND_PERMISSION, (Activity)mContext, Common.REQUEST_CODE );
    }
    ...
    public void onPerformCallClicked(ContactData contactData) {
        if ((Common.checkPermissions(Common.PHONE_CALL_PERMISSION,(Activity)mContext))) {
            Intent callIntent = new Intent( Intent.ACTION_CALL, Uri.fromParts( "tel", contactData.getmPhoneNumber(), null ) );
            mContext.startActivity( callIntent );
        } else
            Common.verifyPermissions( Common.PHONE_CALL_PERMISSION,(Activity)mContext,Common.REQUEST_CODE );
    }
